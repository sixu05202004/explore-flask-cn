...
SQLAlchemy 是一个 ORM（对象关系映射）。它基本上是在我们的数据库中执行的原始的 SQL 查询的之上的抽象层。它为一个长长的列表的数据库引擎提供一致的 API。其中最流行的数据库引擎包括 MySQL, PostgreSQL 和 SQLite。它使得在我们模型和数据库之间很容易地移动数据，并且它真的很容易地去做一些其它的事情，比如更换数据库引擎和迁移我们的模式。
...
``Column``，``Integer``，``String``，``Model`` 以及其它的 SQLAlchemy 类可以从由 Flask-SQLAlchemy 定义的 ``db`` 对象中引用。我们已经定义了一个模型用来存储我们飞行器引擎当前的状态。每一个引起过有一个 id，一个标题以及一个推力水平。

我们还需要把一些数据库信息添加到我们配置中。我们使用一个实例文件夹为了保持机密的配置变量不在版本控制中，所以我们将要把数据库的配置放入 *instance/config.py*。
...
.. note::
   ...
   你的数据库 URI 会根据你使用的数据库引擎会是不同的。请参阅 `SQLAlchemy 官方文档 <http://docs.sqlalchemy.org/en/latest/core/engines.html?highlight=database#database-urls>`_ 获取更多的内容。

初始化数据库 
~~~~~~~~~~~~~~

现在数据库应经配置好了并且我们已经定义一个模型，我们可以初始化数据库。这一步基本上涉及到从模型定义中创建数据模式。
...
现在多亏了 SQLAlchemy，我们的表已经在我们的配置中指定的数据库中创建。

Alembic 迁移数据库
~~~~~~~~~~~~~~~~~~~~~

数据库的模式不是一成不变的。例如，我们可能要给我们的引擎表添加一个 ``last_fired`` 字段。如果我们没有任何数据的话，我们只要更新模型和再次运行 ``db.create_all()``。然而，如果我们已经在表中记录了 6 个月的引擎的数据，我们可能不希望从头开始。这就是数据库迁移的工作。

Alembic 是一个专门为 SQLAlchemy 使用的数据库迁移工具。它能够让我们保持我们的数据模式的版本历史，以便我们后面可以升级到一个新的模式以及甚至退回到一个老的版本。
...
*alembic/* 目录下有在不同版本之间迁移我们数据的脚本。同样也有包含配置信息的一个 *alembic.ini* 文件。
